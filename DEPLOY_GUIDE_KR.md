...
### 4-1. Work 모드 (선택)
업무용으로 별도 폴더에 날짜별로 정리하고 싶으면:
1.  Google Drive에 업무 전송용 부모 폴더를 하나 만들고 폴더 ID를 복사합니다.
2.  Streamlit Secrets에 다음 줄을 추가합니다:
    ```toml
    gdrive_folder_id_work = "여기에_업무용_폴더_ID"
    ```
3.  앱에서 **💼 Send to Work** 토글을 켜면, KST 기준 오늘 날짜의 하위 폴더(예: `260507`)가 자동으로 만들어지고 그 안에 파일이 저장됩니다. 같은 날 다시 보내면 같은 폴더에 누적됩니다.
